...
class RegistrationState(StatesGroup):
    ...
    email = State()
    age = State()
    # зачем тут balance, если он не будет использоваться в коде?
    balance = State()
...
async def set_username(msg, state):
    ...
    if is_included(msg.text):
        await msg.answer("Данное имя пользователя уже используется")
        await state.clear()
    else:
...
async def set_age(msg: Message, state):
    await state.update_data(age=msg.text)
    data = await state.get_data()
    add_user(data['username'], data['email'], data['age'])
    await msg.answer("Регистрация выполнена успешно")
    await state.clear()
